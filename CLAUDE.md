...
3. 调用 add_memory(content="修复 Qdrant 并发锁...", layer="fact", category="event", confidence=0.95)
...
| 完成了重构 | fact | event |
| 发现了重要信息 | fact | 按内容 |
| Sprint/里程碑完成 | fact | event |
| 一轮对话结束（有实质进展） | session | event |
...
## 三层记忆规则
1. **宪法层**：核心身份，仅照护者可改，**需三次CLI确认**
2. **事实层**：长期记忆，AI 可提取，人工可修正
3. **会话层**：短期对话，24h 后归档
...
def generate_observation(work_result):
    return {
        "content": "用一句话描述发生了什么",
        "layer": "fact",  # fact | session（宪法层禁止AI写入）
...
|------|-------|----------|------|
| 重要决策 | fact | event | "决定使用 Qdrant 作为向量数据库" |
| Bug 修复 | fact | event | "修复了 search_memory 的空指针问题" |
| 架构变更 | fact | item | "添加了三层记忆模型" |
| 发现关键信息 | fact | 按内容 | "患者提到女儿下周要出差" |
| 会话摘要 | session | - | "本次会话讨论了 MCP 集成方案" |
...
| type | 默认 layer | 何时使用 |
|------|-----------|---------|
| `decision` | fact | 架构/技术决策 |
| `bugfix` | fact | Bug 修复 |
| `refactor` | fact | 重构记录 |
| `discovery` | session | 发现/探索（待验证） |
| `note` | session | 临时笔记 |
...
✅ 记忆已写入（layer=fact, confidence=0.9）
```
