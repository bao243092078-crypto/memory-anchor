...
>
> **核心澄清**："阿默斯海默症"是把 AI 比作失忆患者的**隐喻**，不是为真实患者设计。
...
### 1.1 定位（⚠️ 已废弃）
~~**阿默斯海默记忆助手**是一个为阿尔茨海默症患者及其照护者设计的记忆辅助 Skill。~~

**当前定位**：Memory Anchor 是 AI 编程助手的**外挂海马体**，解决上下文压缩导致的"失忆"问题。它不是存储引擎，而是**记忆行为的抽象层**，负责定义"该怎么想起、怎么记录、怎么治理"。
